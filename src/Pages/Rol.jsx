...
const Roles = () => {
    ...
    const openNew = () => {
        setRol({ id: null, funcion: '', responsabilidad: '' });
    ...
    const saveRol = async () => {
        ...
        if ( rol.nombre && rol.funcion && rol.responsabilidad && rol.responsabilidad.trim()) {
            try {
                if (rol.id) {
        ...
        <div className="container mt-4">
            <div className="card shadow p-4">
                ...
                <DataTable value={roles} className="p-datatable-sm">
                    ...
                    <Column field="funcion" header="Funcion"></Column>
                ...
                <Dialog visible={productDialog} style={{ width: '30rem', paddingBottom: '0' }} header={`${rol ? 'Editar' : 'Nuevo'} Tipo de Ubicación`} modal className="p-fluid" onHide={hideDialog}>
                    ...
                    <div className="p-field">
                        ...
                        <InputText id="descripcion" value={rol?.descripcion || ''} onChange={(e) => onInputChange(e, 'descripcion')} required autoFocus className="form-control" />
                        {submitted && !rol?.descripcion && <small className="p-error">El nombre es requerido.</small>}
                    </div>
                ...
                <Dialog visible={deleteProductsDialog} style={{ width: '30rem' }} header="Confirmación" modal footer={deleteProductsDialogFooter} onHide={() => setDeleteProductsDialog(false)}>
                    <div className="p-d-flex p-ai-center p-p-3">
                        ...
                        {rol && (
                            <span>¿Seguro que quieres eliminar el tipo de ubicación <b>{rol.nombre}</b>?</span>
                        )}
                    </div>
                </Dialog>
            </div>
