...
const TipoArticulos = () => {
    ...
    const saveTipoArticulo = async () => {
        ...
        if ( tipoArticulo.nombre && tipoArticulo.nombre.trim()) {
            try {
                if (tipoArticulo.id) {
                    await axios.put(`http://127.0.0.1:8000/api/tipo-articulo/${tipoUbicacion.id}`, tipoUbicacion);
                } else {
                    await axios.post(`http://127.0.0.1:8000/api/tipo-articulo`, tipoUbicacion);
                }
                setProductDialog(false);
